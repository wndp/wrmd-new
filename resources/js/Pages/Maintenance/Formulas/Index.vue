...
const search = () => debounce(function() {
    if (form.search === '') {
        ...
        form.get(route('maintenance.formulas.index'));
    }
}, 500)();
</script>
        ...
        <div class="shadow sm:rounded-md sm:overflow-hidden">
          <div class="bg-white py-6 px-4 sm:p-6">
            <h3 class="text-lg leading-6 font-medium text-gray-900">
              {{ __('Prescription Formulary') }}
            </h3>
            <p class="mt-2 text-sm text-gray-500">{{ __("Use the prescription formulary to create your own prescription protocols. This will help your organization's users have a faster and more reliable way to write your most common prescriptions.") }}</p>
          </div>
        </div>
            ...
            <div class="flex rounded-md shadow-sm">
              <div class="relative flex-grow focus-within:z-10">
                ...
                <input
                  ...
                  :placeholder="`${__('Search formulas')}...`"
                  @keyup="search"
                >
              </div>
              <PrimaryButton
                class="rounded-l-none"
                @click="$inertia.get(route('maintenance.formulas.create'))"
              >
                <PlusIcon
                  class="h-5 w-5"
                  aria-hidden="true"
                />
                <span class="ml-2 whitespace-nowrap">{{ __('New Formula') }}</span>
              </PrimaryButton>
            </div>
          </div>
        </div>
