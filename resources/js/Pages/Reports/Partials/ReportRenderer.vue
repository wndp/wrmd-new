...
export default {
    ...
    mounted() {
        window.Echo.private(`device.${this.$cookie.getCookie('device-uuid')}`)
    ...
    unmounted() {
      window.Echo.leave(`device.${this.$cookie.getCookie('device-uuid')}`);
    },
    methods: {
        response(format, url) {
            this.format = format;
            ...
            if (format === 'pdf') {
                ...
                axios({
                    ...
                    FileSaver.saveAs(new Blob([response.data]), this.fileName);
                });
            }
